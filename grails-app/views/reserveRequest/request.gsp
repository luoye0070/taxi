...
<g:if test="${reserveInstance}">
    <div class="row" style="margin: 5px;margin-top: 0px;">
        <div class="panel panel-default">
            <div class="panel-heading">预约乘车请求</div>
            <div class="panel-body">
                <form action="${createLink(controller: "customerReserveOfWeb", action: "apply")}" method="POST">
                    <input type="hidden" name="nickName" value="${reserveInstance?.nickName}"/>
                    ...
                    <div class="form-group">
                        ...
                        <g:textField class="form-control" name="phoneNum" maxlength="11"
                                     value="${reserveInstance?.phoneNum}"/>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <g:textField class="form-control Wdate" name="time" value="${reserveInstance?.time}" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"/>
                    </div>

                    <div class="form-group">
                        <label for="route">
                            <g:message code="reserve.route.label" default="Route"/>
                        </label>
                        <g:select class="form-control" id="route" name="routeId" from="${taxi.Route.list()}"
                                  optionKey="id" optionValue="name" value="${reserve?.route?.id}"/>
                    </div>

                    <g:submitButton class="btn btn-default" name="submit" value="预约"></g:submitButton>
                </form>
            </div>
        </div>
    </div>
